    ...
    public class Part2
    {
        public static void Run()
        {
            ...
            for (int i = 3; i < 1000; i++)
            {
                ...
                if (RunGame(map, elfs, goblins, i, out var result))
                {
        ...
        private static bool RunGame(char[][] map, List<Player> elfs, List<Player> goblins, int elfAttackPower, out int result)
        {
            var rounds = 0;
            while (elfs.Count != 0 && goblins.Count != 0)
            {
                ...
                foreach (var player in playingOrder)
                {
                    if (!player.IsDead)
                    {
                        ...
                        if (opponent != null)
                        {
                            var attackPower = player.Type == 0 ? elfAttackPower : 3;
                            if (opponent.HitPoints > attackPower)
                            {
            ...
            result = (rounds - 1) * elfs.Sum(e => e.HitPoints);
            return true;
        }
